...
- **ASCVD 风险分层**（指南图1 完整实现）
  - 二级预防：严重 ASCVD 事件（4类，支持同类事件复发）与高危险因素（8项）→ 超高危 / 极高危
...
- 分层依据逐条展示，可一键复制结果文本
- 校验年龄、血脂和必要信息；超出成人流程或图表范围时提示临床评估
- v0.1.2 核对记录及适用边界见 [REVIEW.md](REVIEW.md)
...
│  ├─ App.tsx               # 界面
│  └─ App.css               # 样式
├─ scripts/logic-test.ts    # 25 个场景 + 68 项图1及边界检查
